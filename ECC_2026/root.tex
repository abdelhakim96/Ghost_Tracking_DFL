...
This work presents a control framework that enables a multicopter equipped with a three-axis camera gimbal to emulate the flight dynamics of a fixed-wing aircraft. The goal is to provide an operationally simple platform for training and simulation that avoids the aerodynamic constraints of fixed-wing vehicles, such as minimum airspeed and nonholonomic constraints.
...
The framework is evaluated on representative fixed-wing manoeuvres, including a level flight, a vertical loop, a barrel roll, an Immelmann turn, a Split-S, and a Cuban-eight. %Tracking performance is quantified via translational mean absolute error and attitude geodesic error between the fixed-wing reference and the quadcopter–gimbal system.
Results show high-fidelity emulation under nominal conditions, with the camera-orientation geodesic error driven to the numerical-precision floor over full $360^\circ$ aerobatic rotations. While evaluations are conducted in simulation, the approach establishes a practical path toward hardware deployment for pilot training, autonomy research, and controller benchmarking.
...
\caption{Multicopter with three-axis gimbal shown with its reference frames, control inputs (in red) and output variables (in blue).}
...
The multicopter is equipped with a camera mounted on a 3-axis roll-pitch-yaw gimbal with joint angles $\phi_\text{G}$, $\theta_\text{G}$ and $\psi_\text{G}$ and quaternion $\mathbf{q}_\text{G} = \mathbf{q}_x(\phi_\text{G}) \otimes \mathbf{q}_y(\theta_\text{G}) \otimes \mathbf{q}_z(\psi_\text{G}) \in \mathbb{H}_1$, which maps from the multicopter's camera frame $\mathcal{F}_\text{MC}$ to $\mathcal{F}_\text{M}$. Let the camera lever arm in $\mathcal{F}_\text{M}$ be $\mathbf{t}_\text{G} \in \mathbb{R}^3$.
% The state of a multicopter with a two-axis gimbal is
The state of a multicopter with a three-axis gimbal is
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
...
% \end{equation}
\begin{equation}
\mathbf{u}_\text{G} = \begin{bmatrix} \dot\phi_\text{G} & \dot\theta_\text{G} & \dot\psi_\text{G} \end{bmatrix}^\top.
\label{eq:gimbal_u}
\end{equation}
%%%%%%%%%%%%%%%%%%%%
...
\begin{remark}
With a 3-axis gimbal ($\mathbf{S} \equiv \mathbf{I}_3$), the result holds globally. The implementation evaluated in Section~\ref{sec:results} adopts this configuration, so the singular set in Assumption~\ref{ass:non_singularity} is reduced to the multicopter's own attitude singularity ($q_0 = 0$, i.e.\ fully inverted body) and to thrust loss.
\end{remark}
...
For the system~(\ref{eq:quad_dynamics}) with the 3-axis gimbal, the output vector is chosen as
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
\mathbf{y}_\text{M} = \begin{bmatrix} x_\text{M} & y_\text{M} & z_\text{M} & q_{\text{M},3} & \phi_\text{G} & \theta_\text{G} & \psi_\text{G} \end{bmatrix}^\top,
\label{eq:y_M_DFL}
\end{equation}
%%%%%%%%%%%%%%%%%%%%
where $q_{\text{M},3}$ is the third (vector) component of $\mathbf{q}_\text{M}$ and acts as a smooth yaw proxy for the multicopter body; using a quaternion component instead of $R_{\text{bw}}(2,1) = \sin(\psi_\text{M})\cos(\theta_\text{M})$ avoids the degeneracy of the Euler-angle yaw output at large pitch. It can be shown that $\mathbf{r} = \begin{bmatrix} 2 & 2 & 2 & 1 & 1 & 1 & 1 \end{bmatrix}$ and $N = 15$~\cite{sarabakha2025data}. Since $\sum_{i=1}^{7} r_i = 10 \neq 15$, the condition~(\ref{eq:condition}) is not satisfied and the input-output decoupling problem is not solvable for~(\ref{eq:quad_dynamics}) by static state feedback. In fact, $\Delta(\mathbf{x}_\text{M})$ is singular for any $\mathbf{x}_\text{M}$ and is not invertible, because $\mathbf{a}_\text{M}$ is affected only by thrust $T$ and not by other control inputs. In order to get $\Delta(\mathbf{x})$ non-singular, the appearance of $T$ should be delayed to higher order derivatives of $\mathbf{a}_\text{M}$. This can be achieved by a dynamic compensator which incorporates an additional set of state variables $\boldsymbol{\chi}$ to achieve higher relative degrees~\cite{Mistler2001ROMAN}.
...
The input-output decoupling problem is solvable for the system (\ref{eq:quad_dynamics}) by means of a dynamic feedback control law, if it is solvable via static feedback for the extended system (\ref{eq:new_quad_dynamics}). Since the extended system has $\hat{\mathbf{r}} = \begin{bmatrix} 4 & 4 & 4 & 2 & 1 & 1 & 1 \end{bmatrix}$ and $\hat{N} = 17$, the condition~\eqref{eq:condition} is satisfied. Thus, the input-output decoupling problem is solvable for the system (\ref{eq:new_quad_dynamics}) by means of a dynamic feedback control law as in~\eqref{eq:feedback_control}:
%%%%%%%%%%%%%%%%%%%%
\begin{equation}
...
\end{equation}
The gimbal references $(\phi_\text{G}, \theta_\text{G}, \psi_\text{G})$ that close the orientation channel are extracted from the relative quaternion $\mathbf{q}_\text{rel} = \bar{\mathbf{q}}_\text{M} \otimes \mathbf{q}_\text{A}$ in the X-Y-Z Euler sequence consistent with $\mathbf{q}_\text{G}$, and their rate feed-forward is obtained by inverting the body-frame gimbal Jacobian $\mathbf{J}_\text{G}(\phi_\text{G}, \theta_\text{G})$ whose determinant is $\cos\theta_\text{G}$, so the only kinematic singularity of the extended system is the multicopter's own attitude inversion. Because the three-axis gimbal absorbs the full $SO(3)$ camera attitude, the multicopter is free to hold its body yaw at zero (i.e.\ $q_{\text{M},3} = 0$) and no yaw schedule is required on the drone.
