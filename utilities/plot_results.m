...
for i = 1:length(t)
    ...
    % --- Store Actual Body-Frame Gimbal Angles ---
    phi_g(i) = phi_g_raw(i);
    theta_g(i) = theta_g_raw(i);
end
...
% --- Debugging Comparison Plot ---
fig_debug = figure('Name', 'Global Roll Angle Comparison', 'NumberTitle', 'off');
plot(t, gimbal_global_roll_hist * 180/pi, 'b-', 'LineWidth', 1.5);
hold on;
plot(time_hist, gimbal_roll_from_history * 180/pi, 'r--', 'LineWidth', 1.5);
plot(t, drone_global_roll_hist * 180/pi, 'g-.', 'LineWidth', 1.5);
grid on;
title('Comparison of Global Roll Angle Calculation');
xlabel('Time (s)');
ylabel('Angle (degrees)');
legend('Gimbal Roll (Post-Proc)', 'Gimbal Roll (Sim)', 'Drone Roll');
